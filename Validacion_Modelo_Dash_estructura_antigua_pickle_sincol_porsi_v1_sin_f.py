...
def main():
    file = open("Codigo Antiguo/v.obj", 'rb')
    ...
    file = open("Codigo Antiguo/v1.obj", 'rb')
    ...
    model.CantidadPedir = pe.Var(model.i, model.c, model.t, domain=pe.Integers,bounds=(0, 100))



    # model.CantidadPedir2 = pe.Var(model.i, model.c, model.t, domain=pe.Integers, bounds=(0,None) )
    # model.ContenedoresCamion = pe.Var(model.c,model.t,model.i, domain=pe.Integers, bounds=(0,100))


    model.Stock=pe.Var(model.i,model.t, bounds=(0,5000))





    # model.K=pe.Var(model.i,model.c,model.t ,domain=pe.Integers)
    ...
    model.A=pe.Var(model.c,model.t, domain=pe.Integers, bounds=(0,13))
    model.B=pe.Var(model.c, model.t, domain=pe.Integers, bounds=(0,13))
    # model.A=pe.Var(model.c,model.t, domain=pe.Integers, bounds=(0,13))
    # model.B=pe.Var(model.c, model.t, domain=pe.Integers, bounds=(0,13))
    ...
    # $$Stock(i,t)=Stock(i,t-1)+\sum_{c,col}2CantidadPedir(i,c,t,col)PiezasCont[i] -Demanda(i,t)$$

    for i in model.i:
            lhs=model.Stock[i,1]
            rhs=model.StockInicial[i]-model.demanda[i,1]+num_col_camion*sum(model.CantidadPedir[i,c,1]*model.PiezasCont[i] for c in model.c  )
    ...
    for t in list(model.t)[1:]:
        for i in model.i:
                lhs=model.Stock[i,t]
                rhs=model.Stock[i,t-1]-model.demanda[i,t]+num_col_camion*sum(model.CantidadPedir[i,c,t]*model.PiezasCont[i] for c in model.c )
                model.stocks.add(lhs==rhs)
    ...
    #             for t in model.t:
    #
    #                     lhs=model.CantidadPedir2[i,c,t]
    #                     rhs=model.K[i,c,t]
    ...
    #                 rhs=model.CantidadPedir2[i,c,t]
    ...
    for c in model.c:
        for t in model.t:
                ...
                rhs=(model.A[c,t]*1600+model.B[c,t]*1000)
    ...
    for c in model.c:
        for t in model.t:
                ...
                lhs=sum(model.CantidadPedir[i,c,t] for i in model.i)
                rhs=(model.A[c,t]*4+model.B[c,t]*3)
    ...
    for c in model.c:
        for t in model.t:
                ...
                lhs=sum(model.CantidadPedir[i,c,t]*float(int(model.LargoCont[i])==int(1600)) for i in model.i)
                rhs=model.A[c,t]*4
    ...
    for c in model.c:
        for t in model.t:
                ...
                lhs=sum(model.CantidadPedir[i,c,t]*float(int(model.LargoCont[i])==int(1000)) for i in model.i)
                rhs=model.B[c,t]*3
    ...
    l=list(range(25))
    for i in model.i:
        for t in list(model.t)[:len(list(model.t))-dias_stock_cobertura]:
            lhs=model.Stock[i,t]
    ...
    for t in model.t:
        for c in model.c:
            lhs=(sum(model.CantidadPedir[i,c,t] for i in model.i  if int(model.LargoCont[i])==int(1600))*750*1200*1600+
                                         +(sum(model.CantidadPedir[i,c,t] for i in model.i  if int(model.LargoCont[i])==int(1000)))*1000*1000*1200)/(3000*2400*13600)
            rhs=0*model.Y[c,t]
    ...
    # solver.options["lp/threads"]="32"
    # solver.options["lp/resolvealgorithm"]="p"
    # lp / initalgorithm = d
    # lp / resolvealgorithm = d

    # solver.options["concurrenlp/initalgorithmtopt"]=32
    # solver.options["set separating emphasis"]="fast"

    # solver.options["set heuristic emphasis"]="aggressive"
    # solver.options["set emphasis"]="hardlp"
    # solver.options["set presolving emphasis"]="aggressive"
    # # solver.options["MIPFocus"] = 0
    # solver.options["MIPGap"] = 0.01
    ...
    for t in model.t:
        pro=[]
        for i in model.i:
            pro.append(sum(model.CantidadPedir[i,c,t].value  for c in model.c ))
    ...
    for t in model.t:
        pro=[]
        for i in model.i:
            pro.append(sum(model.CantidadPedir[i,c,t1].value  for c in model.c  for t1 in list(model.t)[0:t]))
