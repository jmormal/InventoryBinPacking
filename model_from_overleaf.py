...
# \textit{X$_{kfwt}$}     & Binary variable indicating   whether the row \textit{f} of the truck \textit{k} is carrying containers of width \textit{w} at time \textit{t}     \\ \hline
    ...
    print("cardinal del producto de todos los conjuntos" , len(Q)*len(M)*len(vI)*len(V)*len(Y)*len(X))
...
# \end{equation}
    ...
    for i in I:
        for k in K:
            for t in T:
                model.Add(Q[i,k,t]==sum(M[i,k,t,b,f]*u[i,b] for b in B for f in F  if (i,b) in u))
...
# \end{equation}
    ...
    for b in B:
        for k in K:
            for t in T:
                    for f in F:
                        model.Add(sum(M[i,k,t,b,f] for i in I  )==V[k,b,f,t]*math.floor(K[k].Height/B[b].Height))
# \begin{equation}
...
#
    for k in K:
        for t in T:
            model.Add(sum(M[i,k,t,b,f]*B[b].Height*B[b].Length*B[b].Width for i in I for b in B for f in F )>=int(K[k].alpha*K[k].Volume)*Y[k,t])
# \begin{equation}
...
# \end{equation}
    ...
    for k in K:
        for f in F:
            for t in T:
                if f+1 in F:
                    model.Add(sum(X[k,f,w,t] for w in W)>=sum(X[k,f+1,w,t] for w in W))



    # \begin{equation}
    #  V_{b k 1 t} = V_{b k f t} \forall b k f t
    # \end{equation}
    ...
    #         for f in F:
    #             for t in T:
    #                 model.Add(V[k,b,f,t]==V[k,b,1,t])


    #
    # \begin{equation}
    # \color{blue}
    #  M_{i k t b f} =  M_{i k t b 1} \forall i k t b f
    # \end{equation}
    ...
    #             for b in B:
    #                 for f in F:
    #                     model.Add(M[i,k,t,b,f]==M[i,k,t,b,1])
    ...
    for k in K:
        for t in T[1:]:
            for f in F:
                for w in W:
                    model.Add(X[k,f,w,t]<=X[k,f,w,t-1])
            model.Add(
                sum(M[i,k,t1,b,f] for i in I for b in B for f in F for t1 in T if t<t1)
                      <=
                      sum(M[i,k,t1,b,f] for i in I for b in B for f in F for t1 in T if t<=t1))

    model.Add(sum(Q[i,k,t] for i in I for k in K for t in T)<=int(1.25*sum(D[i,t] for i in I for t in T)))


    for k in K:
        for t in T[1:]:
            model.Add(sum(Q[i,k,t1] for i in I for t1 in T if t<t1)
                      <=
                      sum(Q[i,k,t1] for i in I for t1 in T if t<=t1))
    for t in T[1:]:
        for i in I:
            model.Add(sum(Q[i,k,t1] for k in K for t1 in T if t < t1 )<=
                      sum(Q[i,k,t1] for k in K for t1 in T if t <= t1 ))
...
# \end{equation}
    ...
    solver.parameters.max_time_in_seconds = int(2 * 60)
    ...
    if status == cp_model.OPTIMAL:
        ...
        # print("Numero de camiones:", sum(solver.Value(vY[c, t]) amiones for t in dias))
