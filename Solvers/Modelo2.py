...
class Solver():
    ...
    def update_data(self,path=None, folder="default", now = datetime.datetime.now()  , planning_days=10):
        ...
        def rule_ra(model, i, j, k, t):
            if i!=j:
                return model.x[i,k,t]+sum(C[c].Length*model.lx[c,i,k,t]+C[c].Width*model.wx[c,i,k,t]+C[c].Height*model.hx[c,i,k,t]
        ...
        def rule_rb(model, i, j, k, t):
            if i!=j:
                return model.x[j,k,t]+sum(C[c].Length*model.lx[c,j,k,t]+C[c].Width*model.wx[c,j,k,t]+C[c].Height*model.hx[c,j,k,t]
                                          for c in model.C) <= model.x[i,k,t]+(1-model.b[i,j,k,t])*M
            else:
                return pe.Constraint.Skip
        ...
        # Restricciones xy
        # \begin{align}
        # &\sum_{c \in C}(l x_{cikt}+l y_{cikt}+l z_{cikt}+w x_{cikt}+w y_{cikt}+w z_{ikt}+h x_{cikt}+h y_{cikt}+h z_{cikt})=3*X^i_{kt}& \forall i, t\\
        def rule_xy(model, i, k, t):
            return sum(model.lx[c, i, k, t] + model.ly[c, i, k, t] + model.lz[c, i, k, t] + model.wx[c, i, k, t] + model.wy[
                c, i, k, t] + model.wz[c, i, k, t] + model.hx[c, i, k, t] + model.hy[c, i, k, t] + model.hz[c, i, k, t] for
                       c in model.C) \
                   == 3 * model.Xi[i, k, t]

        model.rule_xy = pe.Constraint(model.I, model.K, model.T, rule=rule_xy,
                                      doc="Restricción que asegura que se respeten las dimensiones del camión en la dirección z")

        # & \sum_{c\in C} hz_{ickt}=1*X^i_{kt} & \forall i, t
        def rule_hz1(model, i, k, t):
            return sum(model.hz[c, i, k, t] for c in model.C) == model.Xi[i, k, t]

        model.rule_hz1 = pe.Constraint(model.I, model.K, model.T, rule=rule_hz1,
                                       doc="Restricción que asegura que se respeten las dimensiones del camión en la dirección z")
        ...
        # Restricciones Contacto entre cajas
        # \begin{align}
        # &x_{ikt}+\sum_{c \in C} (p_{ic} l x_{cikt}+ q_{ci}wx_{cikt}+ r_{ci}hx_{cikt})+\left(1-e_{i j k t}\right) M \geq x_{jkt} & \forall i, j \in I: i \neq j, t \\

        def rule_contacto(model, i, j, k, t):
            if i != j:
                return model.x[i, k, t] + sum(
                    C[c].Length * model.lx[c, i, k, t] + C[c].Width * model.wx[c, i, k, t] + C[c].Height * model.hx[
                        c, i, k, t] for c in model.C) + (1 - model.e[i, j, k, t]) * M >= model.x[j, k, t]
            else:
                return pe.Constraint.Skip

        model.rule_contacto = pe.Constraint(model.I, model.I, model.K, model.T, rule=rule_contacto,
                                            doc="Restricción que asegura que se respeten las dimensiones del camión en la dirección z")

        # \begin{split}
        #     &x_{ikt}+\sum_{c \in C} (p_{ic} l x_{cikt}+ q_{ci}wx_{cikt}+ r_{ci}hx_{cikt})-x_{jkt}+\left(1-e_{i j k t}\right) M \geq \\
        #     & \quad \quad \quad  \sum_{c \in C} (p_{jc} l x_{cjkt}+ q_{cj}wx_{cjkt}+ r_{cj}hx_{cjkt} ) \sqrt{\alpha}

        def rule_contacto2(model, i, j, k, t):
            if i != j:
                return model.x[i, k, t] + sum(
                    C[c].Length * model.lx[c, i, k, t] + C[c].Width * model.wx[c, i, k, t] + C[c].Height * model.hx[
                        c, i, k, t] for c in model.C) - model.x[j, k, t] + (1 - model.e[i, j, k, t]) * M >= \
                       sum(C[c].Length * model.lx[c, j, k, t] + C[c].Width * model.wx[c, j, k, t] + C[c].Height * model.hx[
                           c, j, k, t] for c in model.C) * alpha
            else:
                return pe.Constraint.Skip

        model.rule_contacto2 = pe.Constraint(model.I, model.I, model.K, model.T, rule=rule_contacto2,
                                                doc="Restricción que asegura que se respeten las dimensiones del camión en la dirección z")
        # \end{split} & \forall i, j \in I: i \neq j, t
        # \end{align}
        # \begin{align}
        # &y_{ikt}+\sum_{c \in C} (q_{ci} w y_{cikt}+ p_{ic}ly_{cikt}+r_{ci}zy_{cikt})\left(1-l y_{cikt}\right)+\left(1-e_{i j k t}\right) M \geq y_k & \forall i, j \in I: i \neq j, t \\

        def rule_contacto3(model, i, j, k, t):
            if i != j:
                return model.y[i,k, t] + sum(
                    C[c].Width * model.wy[c, i, k, t] + C[c].Length * model.ly[c, i, k, t] + C[c].Height * model.hy[
                        c, i, k, t] for c in model.C) + (1 - model.e[i, j, k, t]) * M >= model.y[j,k, t]
            else:
                return pe.Constraint.Skip

        model.rule_contacto3 = pe.Constraint(model.I, model.I, model.K, model.T, rule=rule_contacto3,
                                             doc="Restricción que asegura que se respeten las dimensiones del camión en la dirección z")

        # \begin{split}
        # &y_{ikt}+\sum_{c \in C} (q_{ci} w y_{cikt}+ p_{ic}ly_{cikt}+r_{ci}zy_{cikt})-y_k+\left(1-e_{i j k t}\right) M \geq \\  &\sum_{c \in C} (q_{cj} w y_{cjkt}+ p_{jc}ly_{cjkt}+r_{cj}zy_{cjkt})\sqrt{\alpha}
        # \end{split}&\forall i, j \in I: i \neq j, t

        def rule_contacto4(model, i, j, k, t):
            if i != j:
                return model.y[i,k, t] + sum(
                    C[c].Width * model.wy[c, i, k, t] + C[c].Length * model.ly[c, i, k, t] + C[c].Height * model.hy[
                        c, i, k, t] for c in model.C) - model.y[j,k, t] + (1 - model.e[i, j, k, t]) * M >= \
                       sum(C[c].Width * model.wy[c, j, k, t] + C[c].Length * model.ly[c, j, k, t] + C[c].Height * model.hy[
                           c, j, k, t] for c in model.C) * alpha
            else:
                return pe.Constraint.Skip

        model.rule_contacto4 = pe.Constraint(model.I, model.I, model.K, model.T, rule=rule_contacto4,
                                             doc="Restricción que asegura que se respeten las dimensiones del camión en la dirección z")


        # # rule to determine the number of trucks used
        # def rule_num_camiones(model):
        #     return sum(model.Y[k, t] * K[k].Volume for k in model.K for t in model.T) >= sum(abs(sum(D[n,t] for t in model.T)/ min(C[c].Volume for c in model.C))for n in model.N)
        #Funcion objetivo
        ...
        try:
            ...
            print("Status: ", results.solver.status, file=open("results_modelo2_G.txt", "a"))
...
if __name__ == '__main__':
    ...
    # Try to delete the file "results_modelo2_G.txt" if it exists
    try:
        os.remove("results_modelo2_G.txt")
    except OSError:
        pass

    for planning_days in [5]:
