...
class Solver():
    ...
    def solve(self,path=None, folder="default", now = datetime.datetime.now()  , planning_days=10):
        ...
        model.x=pe.Var(model.I, model.K, model.T, within=pe.NonNegativeReals, doc="Coordenadas de la esquina frontal inferior izquierda de la casilla i durante el periodo de planificación t")
        model.y=pe.Var(model.I, model.K, model.T, within=pe.NonNegativeReals, doc="Coordenadas de la esquina frontal inferior izquierda de la casilla i durante el periodo de planificación t")
        ...
        def rule_X2(model,  c, i, k, t):
            return sum(model.Nn[n, c, i, k, t] for n in model.N) == model.Xc[c, i, k, t]*(K[k].Height//C[c].Height)
        ...
        def rule_ra(model, i, j, k, t):
            if i!=j:
                return model.x[i,k,t]+sum(C[c].Length*model.lx[c,i,k,t]+C[c].Width*model.wx[c,i,k,t]
        ...
        def rule_rb(model, i, j, k, t):
            if i!=j:
                return model.x[j,k,t]+sum(C[c].Length*model.lx[c,j,k,t]+C[c].Width*model.wx[c,j,k,t]
        ...
        def rule_rd(model, i, j, k, t):
            if i!=j:
                return model.y[j,k,t]+sum(C[c].Width*model.wy[c,j,k,t]+C[c].Length*model.ly[c,j,k,t]
        ...
        # &a_{i j k t}+b_{i j k t}+c_{i j k t}+d_{i j k t}+e_{i j k t}Nn+f_{i j k t}+(1-X^i_{kt})+(1-X^j_{kt}) \geq 1   &\forall i, j \in I: i \neq j, t , k \label{eq8}

        def rule_abc(model, i, j, k, t):
            if i!=j:
                return model.a[i,j,k,t]+model.b[i,j,k,t]+model.c[i,j,k,t]+model.d[i,j,k,t]\
                       +(1-model.Xi[i,k,t])+(1-model.Xi[j,k,t]) >= 1
            else:
                return pe.Constraint.Skip
        ...
        def rule_dim_1(model, i, k, t):
            return model.x[i,k,t] + sum(model.lx[c,i,k,t] * C[c].Length + model.wx[c,i,k,t] * C[c].Width  for c in model.C)\
        ...
        def rule_orient_1(model, i, k, t):
            return sum(model.lx[c,i,k,t] + model.ly[c,i,k,t]  for c in model.C) == model.Xi[i,k,t]
        ...
        def rule_orient_2(model, i, k, t):
            return sum(model.wx[c,i,k,t] + model.wy[c,i,k,t]  for c in model.C) == model.Xi[i,k,t]
        ...
        def rule_orient_4(model, i, k, t):
            return sum(model.lx[c,i,k,t] + model.wx[c,i,k,t] for c in model.C) == model.Xi[i,k,t]
        ...
        def rule_orient_5(model, i, k, t):
            return sum(model.ly[c,i,k,t] + model.wy[c,i,k,t]for c in model.C) == model.Xi[i,k,t]
