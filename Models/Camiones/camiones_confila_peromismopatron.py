...
def update_data(default_time=60, default_days=True,dias_stock_cobertura=3):
    path = os.getcwd()
    # Change cwd
    ...
    rCajasTotales={(c,t,f):
                       model.Add(
                           sum(vCantidadPedir[(i,c,t,f)] for i in productos)<=(vA[c,t]*4+vB[c,t]*3))
    ...
    rCajasTipoA={(c,t,f):
                     model.Add(
                         sum(vCantidadPedir[(i,c,t,f)]*float(int(piezas_largo_box[i])==int(1600)) for i in productos)<=(vA[c,t]*4))
        for c in camiones for t in dias for f in filas}
    ...
    for c in camiones:
        for t in dias:
            for caja in range(2):
                for f in filas:
                    if caja==0:
                        rCargaMaxima.update({(c,t,caja,f):model.Add(sum(vCantidadPedir[(i,c,t,f)] for i in productos if int(piezas_largo_box[i])==int(1600))==4*vA[c,t])})
                    else:
                        rCargaMaxima.update({(c,t,caja,f):model.Add(sum(vCantidadPedir[(i,c,t,f)] for i in productos if int(piezas_largo_box[i])==int(1000))==3*vB[c,t])})




    rCajasTipoB={(c,t,f):model.Add(sum(vCantidadPedir[(i,c,t,f)]*float(int(piezas_largo_box[i])==int(1000)) for i in productos)<=(vB[c,t]*3)) for c in camiones for t in dias for f in filas}
    stock_seguridad = []
    ...
    #         /(3000*2400*13600))])
                            ...
                            if solver.Value(vCantidadPedir[i, c, t,f]) > 0:
                                sol_dia.append([i,solver.Value(vCantidadPedir[(i,c,t,f)]), solver.Value(vA[(c,t)]), solver.Value(vB[(c,t)]), piezas_largo_box[i]])
                                sol_camiones_total.append([t,k1,i,solver.Value(vCantidadPedir[(i,c,t,f)]), solver.Value(vA[(c,t)]), solver.Value(vB[(c,t)]),
                                                           piezas_largo_box[i],
    ...
    print(df_sol_camiones_total)
    print("Total number of containers with length 1600: ", sum(solver.Value(vCantidadPedir[i,c,t,f]) for f in filas for i in productos for c in camiones for t in dias if int(piezas_largo_box[i])==int(1600)))
    print("Total number of containers with length 1000: ", sum(solver.Value(vCantidadPedir[i,c,t,f]) for f in filas for i in productos for c in camiones for t in dias if int(piezas_largo_box[i])==int(1000)))
